...
import argparse
import json
os.environ['RLTRADER_BASE'] = 'C:/Users/박인창/OneDrive - 한양대학교/1 공모전 결과물 모음/1 AI SPARK 챌린지/emission_trading_model - 복사본' ##윈도우

from programs import settings
from programs import utils
from programs import data_manager
...
if __name__ == '__main__':
    ...
    from programs.learners import ReinforcementLearner, DQNLearner, \
    ...
    for stock_code in args.stock_code:
        ...
        # 최소/최대 단일 매매 금액 설정
        # min_trading_unit = max(int(50000 / chart_data.iloc[-1]['ets_price']), 1)
        # max_trading_unit = max(int(90000 / chart_data.iloc[-1]['ets_price']), 1)

        min_trading_unit = 100
        max_trading_unit = 2000 # 1000
