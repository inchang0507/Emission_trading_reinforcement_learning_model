...
from programs.environment import Environment
from programs.agent import Agent

if os.environ.get('RLTRADER_BACKEND', 'pytorch') == 'pytorch':
    from programs.networks_pytorch import Network, DNN, LSTMNetwork, CNN, Q3
else:
    from programs.networks_keras import Network, DNN, LSTMNetwork, CNN
from programs.visualizer import Visualizer
...
class ReinforcementLearner:
    ...
    def __init__(self, rl_method='rl', stock_code=None, 
        ...
        self.memory_exp_idx = []
        self.memory_avgprice = [] # 평균 매입 가격
        self.memory_amount = [] # 총 비용
        self.memory_totalamount = []
        self.memory_balance = []
    ...
    def reset(self):
        ...
        self.memory_policy = []
        # self.memory_pv = []
        self.memory_avgprice = []
        self.memory_totalamount = []
        self.memory_amount = []
        self.memory_num_stocks = []
        self.memory_exp_idx = []
        self.memory_balance = []
        self.memory_tradingunit = []
        # 에포크 관련 정보 초기화
        self.loss = 0.
        self.itr_cnt = 0
        self.exploration_cnt = 0
    ...
    def run(self, learning=True):
        ...
        for epoch in tqdm(range(self.num_epoches)):
            ...
            for i in tqdm(range(len(self.training_data)), leave=False):
                ...
                self.memory_balance.append(self.agent.balance)
                self.memory_num_stocks.append(self.agent.num_stocks)
                self.memory_avgprice.append(self.agent.avg_buy_price)
                self.memory_totalamount.append(self.agent.total_buy_amount)
                self.memory_amount.append(self.agent.buy_amount)
                self.memory_tradingunit.append(self.agent.trading_unit_f)
            ...
            logging.info("[{}][Epoch {}/{}] Epsilon:{:.4f} "
                "#Expl.:{}/{} #Buy:{} #Hold:{} "
                "#Stocks:{} #needed_eua:{} "
                "#averge_price:{:.4f} #curr_amount:{:.2f} #total_amount:{:.2f} "
                "Loss:{:.6f} ET:{:.4f}".format(
                    self.stock_code, epoch_str, self.num_epoches, epsilon, 
                    self.exploration_cnt, self.itr_cnt,
                    self.agent.num_buy, self.agent.num_hold,
                    self.agent.num_stocks, self.agent.balance,
                    self.agent.avg_buy_price, self.agent.buy_amount, self.agent.total_buy_amount,
            ...
            # 학습 관련 정보 갱신
            max_avg_value = self.agent.avg_buy_price
            if max_avg_value > self.agent.import_low_price:
                epoch_win_cnt += 1
        ...
        # 정보 저장
        chart_df = pd.DataFrame(self.environment.chart_data)
        chart_df['reward'] = self.memory_reward
        chart_df['avg_price'] = self.memory_avgprice
        chart_df['total_amount'] = self.memory_totalamount
        chart_df['action'] = self.memory_action
        chart_df['trading_unit'] = self.memory_tradingunit
        chart_df.to_csv('out_data/result_{}.csv'.format(int(time_end)))
        ...
        with self.lock:
            logging.info("[{code}] Elapsed Time:{elapsed_time:.4f} "
                ...
                max_pv=max_avg_value, cnt_win=epoch_win_cnt))
    ...
    def predict(self, balance=100000):
        # 에이전트 초기 자본금 설정
